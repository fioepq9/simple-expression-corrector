...
def load_data(root, training=True, fix_width=False):
    ...
    if training:
        ...
        if os.path.exists(filename):
            ...
            print('==== Loading data.. ====')
            dataset = Evalset(root, training=True, fix_width=fix_width)
        ...
        if os.path.exists(filename):
            ...
            print('==== Loading data.. ====')
            dataset = Evalset(root, training=False, fix_width=fix_width)
            pickle.dump(dataset, open(filename, 'wb'), True)
...
def ReadData(training=True) -> List[str]:
    if training:
        b, e = 0, 300
    else:
        b, e = 300, 342
    with open('data/eval.txt', encoding='utf8') as f:
        i = 0
        while True:
            line = f.readline()
            if not line:
...
class Evalset(Dataset):
    ...
    def __init__(self, root, training=True, fix_width=False):
        ...
        self.img, self.label = zip(*[(x, y) for x, y in ReadData(training)])
